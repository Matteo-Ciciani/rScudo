...
```{r}
ScudoTrain <- scudoTrain(train, groups = bt[inTrain], nTop = 100, nBottom = 100, alpha = 0.1)
ScudoTrain
```
...
```{r}
netTrain <- scudoNetwork(ScudoTrain, N = 0.2)
```
...
```{r}
scudoPlot(netTrain, vertex.label = NA)
```
...
We can use a `ScudoResults` object and the function `scudoTest` to analyze the testing set. The feature selection is not performed in the testing set. Instead, only the features selected in the training are used in the analysis of the testing set.

```{r}
ScudoTest <- scudoTest(ScudoTrain, test, bt[-inTrain], nTop = 100, nBottom = 100)
ScudoTest
```
...
```{r}
netTest <- scudoNetwork(ScudoTest, N = 0.2)
plot(netTest)
```
...
Cluster with kmeans on the weighted agjacency matrix

```{r}
igraph::E(netTest)$weight <- 2 - igraph::E(netTest)$distance
adj <- igraph::get.adjacency(netTest, attr = "weight")
km <- kmeans(adj, 2)
km$cluster == as.integer(bt[-inTrain])
```

Use community clustering forcing 2 communities:

```{r}
spr <- igraph::cluster_spinglass(netTest, weights = NULL, spins = 2)
plot(spr, netTest)
```
...
```{r}
mol <- phenoData$mol.biol[bt == "B", drop = TRUE][-48]
expDataB <- expData[, bt == "B"][, -48]

ScudoTrain2 <- scudoTrain(expDataB, groups = mol, nTop = 200, nBottom = 200, alpha = 0.05)
ScudoTrain2
```

```{r}
netTrain2 <- scudoNetwork(ScudoTrain2, 0.08)
scudoPlot(netTrain2, vertex.label = NA)
```
...
```{r}
spr2 <- igraph::cluster_spinglass(netTrain2, weights = NULL, spins = 4)
plot(spr2, netTrain2)
```
...
```{r}
feats <- selectedFeatures(ScudoTrain2)
expSel <- expDataB[feats, ]
pcaRes <- prcomp(t(expSel))
pairs(pcaRes$x[, 1:5], col = rainbow(4)[as.integer(mol)])
```

# Do the good ol'thing
...
res <- scudoTrain(bcrneg, bcrneg$mol.biol, 50, 50, 0.01)
```
