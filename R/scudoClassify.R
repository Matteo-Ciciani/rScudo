...
#' using the network generated by \code{\link{scudoTrain}} and
#' \code{\link{scudoNetwork}} as a model.
#'
...
#' Then, the distance matrix is converted in a similarity score matrix.
#'
#' If the argument \code{complete} is \code{TRUE}, the similarity scores between
#' S and all the samples in the training set are aggregated according to groups.
...
#' nodes of that group. The scores are than rescaled dividing them by their
#' sum, in order to obtain values between 0 and 1. The parameter \code{beta}
#' can be used to down-weight the contribution to the classification scores
#' of edges connecting nodes distant form S, both in the weighed and unweighted
#' cases.
#'
