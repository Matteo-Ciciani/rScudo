...
The aim of SCUDO is to perform as a classifier for microarray-based phenotype prediction. The method is based on the idea of a sample-specific signature. The expected result is the emergence of a partitioning of the set of samples in separate groups on the basis of signature similarity. \\
...
Finally, a threshold as the Nth lowest percentile of distances is used in order to produce a map as a graph, where each node represents signatures and each node is connected by an edge corresponding to their respective distance. The map is produced through the use of Cytoscape. 
