...
First, normalization and feature selection are optionally performed. Feature selection is carried out through a Wilcoxon rank sum test or through a Kruskal-Wallis rank sum test depending on the number of sampleGroups considered.
...
```{r}
ScudoTrain <- scudo(train, sampleGroups = bt[inTrain], nTop = 100, nBottom = 100, alpha = 0.1)
ScudoTrain
```
...
The sampleGroups are already well partitioned. We can nonetheless perform the community clustering.
